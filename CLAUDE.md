...
### ⚡ Выкатка — одна команда

```bash
./deploy.sh
```

Единый конвейер для всех сервисов на `vlad-main` (заведён 2026-09-03): в
`tg-claude`, `vladburba-bot` и `landing-page` выкатка называется и работает
одинаково — не надо вспоминать, у кого вебхук, у кого push на сервер, а у кого
pull руками.

Что делает: отказ при незакоммиченных правках → отправка в **оба** хранилища
(`nas` — история, второй адрес — доставка на прод) → подтягивание кода на
сервере проверка контейнера →
проверка ответа сайта снаружи. Каждый шаг
проверяется: скрипт не рапортует об успехе, если шаг не выполнился.

**Автодеплой по вебхуку на сервере остаётся** — он делает pull, проверяет ответ
сайта и сам откатывается на прежний коммит при провале. Скрипт его не заменяет:
он отправляет код и дожидается подтверждения, что нужный коммит доехал и сайт
отвечает. Если вебхук не сработал, скрипт это заметит и подскажет ручную команду.

Команды ниже — справочник для отладки и разовых операций. Штатная выкатка — `./deploy.sh`.
